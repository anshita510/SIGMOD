device_lookup_path = '/home/pi/devic3/newAll_Device_Lookup_with_Probabilities.csv'
...
device_folder = 'Device3'
device_partitions_patientwise = 'Device3/'
...
def load_tphg(patient_id):
    ...
    try:
        if os.path.exists(cache_file):
            print(f"[Device] TPHG cache found for patient {patient_id}. Attempting to load...")
            
            loaded_graph = joblib.load(cache_file)
            print(f"[Device3] TPHG successfully loaded for patient {patient_id}")
            ...
            print(f"[Device3] No TPHG cache found for patient {patient_id}. Returning None.")
            return None
    
    except Exception as e:
        print(f"[Device3] Error occurred while loading TPHG for patient {patient_id}: {e}")
        raise  
