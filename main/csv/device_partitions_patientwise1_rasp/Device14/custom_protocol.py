...
class CustomProtocol:
    ...
    def send_packet(self, packet, target_host, target_port):
        try:
            ...
            print(f"Packet sent to {target_host}:{target_port} - {packet}")
        except Exception as e:
            print(f"Failed to send packet to {target_host}:{target_port}. Error: {str(e)}")
    ...
    def receive_packet(self, timeout=None):
        ...
        try:
            ...
            print(f"Packet received from {addr[0]}:{addr[1]} - {packet}")
            return packet, addr
        except socket.timeout:
            print("Timeout reached while waiting for packet.")
            return None, None
        except Exception as e:
