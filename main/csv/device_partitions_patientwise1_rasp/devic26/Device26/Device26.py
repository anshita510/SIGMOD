device_lookup_path = 'newAll_Device_Lookup_with_Probabilities.csv'
...
filtered_patients_path = 'filtered_patients_updated.csv'
...
def load_tphg(patient_id):
    ...
    if os.path.exists(cache_file):
        print(f"[Device26] TPHG cache loaded for patient {patient_id}")
    ...
    print(f"[Device26] No TPHG cache found for patient {patient_id}.")
    return None
...
def find_next_device(effect, patient_id, sender_device=None, visited_devices=None):
    ...
    lookup_table['device'] = lookup_table['device'].astype(str).str.strip().str.lower()
    relevant_devices = [str(dev).strip().lower() for dev in relevant_devices]
    ...
    for idx, row in filtered_lookup_table.iterrows():
        ...
        if next_device in visited_devices or next_device == device_folder.lower():
            continue
    ...
    if not effect_found:
        ...
        print(device_folder)
        current_device_row = lookup_table[lookup_table['device'] == device_folder.lower()]
    ...
    if tphg is not None:
        ...
        if viterbi_paths:
            for path in viterbi_paths:
                for node in path:
                    current_device_row = filtered_lookup_table[filtered_lookup_table['device'] == device_folder.lower()]
