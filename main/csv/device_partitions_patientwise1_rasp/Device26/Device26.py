device_lookup_path = '/home/pi/devic26/newAll_Device_Lookup_with_Probabilities.csv'
...
filtered_patients_path = '/home/pi/filtered_patients_updated.csv'
filtered_patients = pd.read_csv(filtered_patients_path)
monitorlog_dir = '/home/pi/logs'
os.makedirs(monitorlog_dir, exist_ok=True)

def calculate_transfer_rate(rate, interval=3, interface="lo0", log_file=None):
    """Calculate and log transfer rates for a network interface."""
    import psutil
...
def start_ul_dl_monitoring(patient_id, disease_name, interval=3):
    """Start monitoring UL/DL for a specific patient-disease combination."""
    log_file = os.path.join(monitorlog_dir, f"{patient_id}_{disease_name}_transfer_rate.csv")
    ...
    monitor_thread = threading.Thread(
        ...
        args=(transfer_rate_queue, interval, "lo0", log_file)
    )
...
def stop_ul_dl_monitoring(thread):
    """Stop the UL/DL monitoring thread."""
    if thread.is_alive():
        thread.join(timeout=5)

log_dir = '/home/pi/devic26/'
...
device_folder = 'Device26'
device_partitions_patientwise = 'Device26/'
...
def load_tphg(patient_id):
    ...
    try:
        if os.path.exists(cache_file):
            print(f"[Device26] TPHG cache found for patient {patient_id}. Attempting to load...")
            
            loaded_graph = joblib.load(cache_file)
            print(f"[Device26] TPHG successfully loaded for patient {patient_id}")
            
            if isinstance(loaded_graph, (nx.Graph, nx.MultiDiGraph)):
                print("Edges in the graph:", list(loaded_graph.edges(data=True)))
                return loaded_graph  
            else:
                raise ValueError(f"Unexpected data format in {cache_file}. Expected a NetworkX graph.")

        else:
            print(f"[Device26] No TPHG cache found for patient {patient_id}. Returning None.")
            return None
    
    except Exception as e:
        print(f"[Device26] Error occurred while loading TPHG for patient {patient_id}: {e}")
        raise  
...
def return_to_initiator(chain, initiator, patient_id):
    ...
    protocol.send_packet({
        ...
        'initiator': initiator,
        'final_packet': True  
    }, '172.20.10.2', lookup_table.loc[lookup_table['device'] == initiator, 'port'].values[0])
